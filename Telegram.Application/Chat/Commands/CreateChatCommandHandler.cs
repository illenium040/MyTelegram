...
internal class CreateChatCommandHandler : CommandHandler<CreateChatCommand, ChatEntity>
{
    ...
    public override async Task<Result<ChatEntity>> Handle(CreateChatCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Failure<ChatEntity>(new("CreateChatCommandHandler.InitiatorUser", "User not found"));
        }

        var receiver = await _userRepository.GetByIdAsync(request.ReceiverUser);
        if (receiver is null)
        {
            return Result.Failure<ChatEntity>(new("CreateChatCommandHandler.ReceiverUser", "User not found"));
        }
        ...
        return result.IsFailure ? Result.Failure<ChatEntity>(result.Error) : Result.Success(chat);
    }
}
